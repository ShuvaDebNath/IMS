...
export class DeliveryComponent implements OnInit {
  ...
  GetPIByPID() {
    if (this.SearchFormgroup.invalid) {
      this.markAllTouched(this.SearchFormgroup);
      Swal.fire(
        'Invalid Inputs!',
        'Please complete all required search fields.',
        'info',
      );
      return;
    }
    ...
    this.service.GetInitialData(model).subscribe((res: any) => {
      if (res.status) {
        ...
        if (
          DataSet.Tables1[0].IsMPI == 1 &&
          DataSet.Tables1[0].CR_NO == 'Not Received'
        ) {
          Swal.fire(
            'Info',
            'Cash Receipt Not Received Yet. Please talk with the finance department.',
            'info',
          );
          return;
        }
        ...
        DataSet.Tables1.forEach((item: any) => {
          itemarray.push(
            this.fb.group({
              ...
              Date: [new Date()],
              Ordered: [0],
              Delivered: [0, [Validators.required, Validators.min(1)]],
              Roll: [0, [Validators.required, Validators.min(1)]],
              Remark: [''],
              Chalan_No: [DataSet.Tables2[0].Chalan_No ?? 0],
              Item_ID: [item.Item_ID, [Validators.required]],
              Stock_Location_ID: ['', [Validators.required]],
  ...
  warehouseStockCheck(item: any) {
    ...
    if (item.value.Delivered == 0 || item.value.Delivered == undefined) {
      Swal.fire(
        'info',
        'Deliver qty must greater then 0 to select werehouse',
        'error',
      );
      item.controls.Stock_Location_ID.reset();
      return;
    }
    if (
      item.value.Stock_Location_ID == undefined ||
      ...
      item.controls.Delivered.reset();
      return;
    }
    let model = new GetDataModel();
    model.procedureName = 'usp_Get_Stock_Info';
    model.parameters = {
      Item_ID: item.value.Item_ID,
      Stock_Location_ID: item.value.Stock_Location_ID,
    };
    this.service.GetInitialData(model).subscribe((res: any) => {
      if (res.status) {
        let DataSet = JSON.parse(res.data);
        var stock = DataSet.Tables1[0].StockQuantity;
        if (stock < item.value.Delivered) {
          Swal.fire(
            'info',
            'Dont have inputed goods in this warehouse. please select another warehouse or lower the quantity.',
            'error',
          );
          item.controls.Delivered.reset();
          item.controls.Stock_Location_ID.reset();
