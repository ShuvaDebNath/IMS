...
pageIndex = 1;
    ...
    swal
      ...
      .then((result) => {
        if (result.isConfirmed == true) {
          let param = new GetDataModel();
          param.procedureName = 'usp_Task_Monthly_Delete';
          param.parameters = {
  ...
  ShowTaskDetails(taskReportId: number) {
    ...
    let param = new GetDataModel();
    param.procedureName = '[usp_Task_Monthly_Details]';
    param.parameters = {
      Id: taskReportId,
    };
    ...
    swal.fire({
      ...
      didOpen: () => {
        ...
        excelBtn?.addEventListener('click', () => {
          swal.close();
          this.reportService.PrintTaskMonthlyDetailsReport(item, 'excel', 'F');
        });

        wordBtn?.addEventListener('click', () => {
          swal.close();
          this.reportService.PrintTaskMonthlyDetailsReport(item, 'word', 'F');
        });

        pdfBtn?.addEventListener('click', () => {
          swal.close();
          this.reportService.PrintTaskMonthlyDetailsReport(item, 'pdf', 'F');
        });
      },
    });
  }
