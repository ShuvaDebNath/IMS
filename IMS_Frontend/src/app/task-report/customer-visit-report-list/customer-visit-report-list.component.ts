...
@Component({
  selector: 'app-customer-visit-report-list',
  templateUrl: './customer-visit-report-list.component.html',
  styleUrls: ['./customer-visit-report-list.component.css']
})
export class CustomerVisitReportListComponent {
pageIndex = 1;
  searchText = '';
  length = 100;
  pageSize = 10;
  ...
  ngOnInit(): void {
    var permissions = this.gs.CheckUserPermission('All Customer Visit List');
    ...
    this.title.setTitle('All Customer Visit List');
    ...
    swal
      ...
      .then((result) => {
        if (result.isConfirmed == true) {
          let param = new GetDataModel();
          param.procedureName = 'usp_Task_Client_Visit_Delete';
          param.parameters = {
  ...
  ShowTaskDetails(taskReportId: number) {
    ...
    let param = new GetDataModel();
    param.procedureName = '[usp_Task_Client_Visit_Details]';
    param.parameters = {
      Id: taskReportId,
    };
