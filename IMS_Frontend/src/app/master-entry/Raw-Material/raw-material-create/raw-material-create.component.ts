...
export class RawMaterialCreateComponent {
Formgroup!: FormGroup;
  isEdit = false;
  ...
  constructor(
    ...
    let has = this.activeLink.snapshot.queryParamMap.has('RawMaterialId');
    if (has) {
      this.RawMaterialId =
        this.activeLink.snapshot.queryParams['RawMaterialId'];
      this.isEdit = true;
      this.GetRawMaterialEditDataById();
    } else {
      this.isEdit = false;
    }
  }
  ...
  ngOnInit() {
    var permissions = this.gs.CheckUserPermission('Raw Material Create');
    ...
    this.title.setTitle('Raw Material Create');
    this.GenerateForm();
  ...
  GetRawMaterialEditDataById() {
    var ProcedureData = {
      procedureName: '[usp_RawMaterial_ById]',
      parameters: {
        Item_Id: this.RawMaterialId,
      },
    };
    this.masterEntyService.GetInitialData(ProcedureData).subscribe({
      next: (results) => {
  ...
  saveData() {
    ...
    var tableName = 'tbl_raw_material';
  ...
  updateData() {
    ...
    let condition = { RawMaterial_ID: this.RawMaterialId };
    let tableName = 'tbl_raw_material';
