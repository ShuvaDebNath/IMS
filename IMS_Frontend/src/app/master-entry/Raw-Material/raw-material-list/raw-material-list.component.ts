...
allCustomers: any[] = [];
  ...
  ngOnInit(): void {
    var permissions = this.gs.CheckUserPermission('Raw Material List');
    ...
    this.title.setTitle('Raw Material List');
  ...
  loadAllFinishGoods(): void {
    ...
    const procedureData = {
      procedureName: 'usp_RawMaterial_List',
      parameters: {
      },
    };
    this.getDataService.GetInitialData(procedureData).subscribe({
      next: (results) => {
        ...
        if (results.status) {
          this.allRMList = JSON.parse(results.data).Tables1;
  ...
  onDelete(row: any) {
    const id = String(row?.RawMaterial_ID ?? '');
    ...
    swal
      .fire({
        title: 'Are you sure?',
        text: `Delete Raw Material ${row?.Article_No || ''}?`,
        icon: 'warning',
      ...
      .then((res) => {
        ...
        const tableName = 'tbl_raw_material';
        const whereParams = { RawMaterial_ID: row?.RawMaterial_ID };
        ...
        this.ms.DeleteData(masterEntryModel).subscribe({
          next: () => {
            swal.fire('Deleted!', 'Raw Material deleted successfully.', 'success');
