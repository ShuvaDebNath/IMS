...
export class MaterialCreateComponent {
  ...
  constructor(
    ...
    let has = this.activeLink.snapshot.queryParamMap.has('MaterialId');
    if (has) {
      this.MaterialId =
        this.activeLink.snapshot.queryParams['MaterialId'];
      this.isEdit = true;
      this.GetMaterialEditDataById();
    } else {
      this.isEdit = false;
    }
  }
  ...
  ngOnInit() {
    var permissions = this.gs.CheckUserPermission('Material Create');
    ...
    this.title.setTitle('Material Create');
    this.GenerateForm();
  ...
  GetMaterialEditDataById() {
    var ProcedureData = {
      procedureName: '[usp_material_GetDataById]',
      parameters: {
        id: this.MaterialId,
      },
    };
      ...
      next: (results) => {
        ...
        if (results.status) {
          ...
          tableData.forEach((e: any) => {
            ...
            this.Formgroup.controls.Description.setValue(e.Description);
  ...
  updateData() {
    ...
    let condition = { Material_ID: this.MaterialId };
    let tableName = 'tbl_material';
