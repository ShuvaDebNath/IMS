...
public class AuthRepository : GenericRepository<UserDto>, IAuthRepository
{
    ...
    {
        const string query = "SELECT User_ID, UserName, Password, Role_id, Superior_ID, IsAuthorized, IsSupplier, Supplier_ID FROM tbl_users where UserName = @UserName and Password = @PasswordHash and IsAuthorized=1";
